...
class AdminController extends Controller
{
    ...
    public function showOrders(Order $order, User $user)
    {
        ...
        $allOrders = Order::orderBy('id', 'DESC')->paginate(40);
    ...
    public function editProduct(Request $request, Product $product)
    {
        ...
        if ($images != null) {
            ...
            $dateStringd = date('Ymd');
            $numberad = rand(100, 100000);
            $filePath = $file->move($destinationPath, $dateStringd . $numberad . 'pro' . $file->getClientOriginalName());
            $incomingFields['product_image'] = basename($filePath); // Save image name
        }
...
*/
        ...
        /*
            foreach ($images['image'] as $image) {
                 ...
                 $imageData = $manager->read($image)->encode();
                 Storage::put('/storage/products/' . $filename, $imageData);
...
*/
    ...
    {
        $productsPage = '/account/admin/products';
        $designsPage = '/account/admin/designs';
        $isDesign = $request->input('name') === "Try Before Buy" ? true : false;
        
        $redirectPage = $isDesign ? $designsPage : $productsPage;
        ...
        try {
            $validationRules = [
                ...
                'box_dimension_outer' => 'nullable',
                'flower_type' => 'required',
                'productimage' => 'required|file', // Validate main product image as a file
            ...
            // Ajusta as regras de validação para "Try Before Buy"
            if ($isDesign) {
                $validationRules['event'] = 'nullable';
                $validationRules['material'] = 'nullable';
                $validationRules['shape'] = 'nullable';
                $validationRules['size'] = 'nullable';
            }
            
            // Executa a validação com as regras ajustadas
            ...
            if ($request->file('productimage')->isValid()) {
                ...
                $filePath = $file->move($destinationPath, $dateString . $randomNumber . 'pro' . $file->getClientOriginalName());
                $incomingFields['product_image'] = basename($filePath); // Save image name
            } else {
                return redirect()->back()->withErrors(['productimage' => 'Invalid product image uploaded.']);
            }
            ...
            if ($request->has('color')) {
                foreach ($request->input('color') as $color) {
                    if ($request->hasFile($color)) {
                        foreach ($request->file($color) as $colorFile) {
                            if ($colorFile->isValid()) {
                                $dateString = date('Ymd');
                                $randomNumber = rand(100, 100000);
                                $destinationPath = 'storage/products';
                                $colorImagePath = $colorFile->move($destinationPath, $dateString . $randomNumber . $colorFile->getClientOriginalName());
                                $imageNames[$color] = basename($colorImagePath); // Store image name with its color as key
                            } else {
                                return redirect()->back()->withErrors(['color' => "Invalid color file for color: $color"]);
                            }
                        }
                    }
                }
            ...
            $isDesign ? $incomingFields['hasMessage'] = 1 : $incomingFields['hasMessage'] = 0;
            ...
            return redirect($redirectPage)->with('success', 'Product added successfully!');
        } catch (Exception $e) {
    ...
    public function getDiscount(Request $request, $oid)
    {
        ...
        $request->session()->put('adresses', $adresses);
        ...
        if ($discount != null && $discount_value == '') {
            ...
            if ($request->session()->has('total_price')) {
                ...
                $request->session()->put('full_price', $request->session()->get('full_price') - $request->session()->get('total_price_w_coupon') * ($discount->amount / 100));
    ...
    public function designYourOwn(Product $product)
    {
        ...
        if (empty($product->eachcolor_image)) {
            return response()->json(['error' => 'eachcolor_image is missing or null'], 400);
        }

        $eachcolor = json_decode($product['eachcolor_image']);

        if (is_null($eachcolor)) {
            return response()->json(['error' => 'Invalid JSON in eachcolor_image'], 400);
        }

        $sel_colors = [];
        foreach ($eachcolor as $key => $value) {
            $col = DB::table('colors')->where('id', $key)->first();
            if ($col) {
                $sel_colors[] = $col;
            }
        }
    ...
    public function designYourOwnProductList()
    {
        $collection = Product::where('name', 'Try Before Buy')->paginate(15);
        return view('products', ['products' => $collection, 'title' => 'All Products']);
    }
