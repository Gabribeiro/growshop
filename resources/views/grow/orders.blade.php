...
@section('title', 'Meus Pedidos - Potiguara Grow Shop')
            ...
            <div class="order-section">
                ...
                @if(count($orders) > 0)
                    @foreach($orders as $order)
                        <a href="{{ route('grow.order.detail', ['order' => $order->id]) }}" class="text-decoration-none">
                                ...
                                <div class="d-flex justify-content-between align-items-center mb-2">
                                    <div>
                                        <span class="order-status 
                                            @if(str_contains(strtolower($order->status), 'pago') || str_contains(strtolower($order->status), 'paid'))
                                                status-paid
                                            @elseif(str_contains(strtolower($order->status), 'cancelado') || str_contains(strtolower($order->status), 'canceled'))
                                                status-canceled
                                            @else
                                                status-pending
                                            @endif">
                                            {{ $orderParts[0] ?? $order->status }}
                                        </span>
                                    </div>
                ...
                @else
                    <div class="no-orders">
                        ...
                        <a href="{{ route('grow.products') }}" class="btn btn-success mt-3">Explorar Produtos</a>
                    </div>
                @endif
