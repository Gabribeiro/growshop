...
@section('title', 'Monte seu Grow - Potiguar Grow')
            ...
            <div class="builder-summary">
                ...
                <div class="p-3">
                    ...
                    @forelse($categories as $category)
                        <div class="builder-summary-item @if(isset($selectedComponents[$category->id])) selected @endif">
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                <h6 class="mb-0">{{ $category->name }}</h6>
                                
                                @if(isset($selectedComponents[$category->id]))
                                    @php
                                        $isEmpty = false;
                                ...
                                <div class="selected-component-container">
                                    <div class="d-flex justify-content-between align-items-center">
                                        <div class="d-flex align-items-center">
                                            @if($component['image'])
                                                <img src="{{ asset('storage/' . $component['image']) }}" alt="{{ $component['name'] }}" class="selected-component-img me-2">
                                            @else
                                            ...
                                            @endif
                                            <div>
                                                <div class="text-truncate" style="max-width: 150px;">{{ $component['name'] }}</div>
                                                <div class="component-price">R$ {{ number_format($component['price'], 2, ',', '.') }}</div>
                                            </div>
                                        </div>
                                        <form action="{{ route('grow.builder.remove') }}" method="POST">
                                            @csrf
                    ...
                    <div class="total-price-display">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            ...
                            <span class="total-price-value">R$ {{ number_format($totalPrice, 2, ',', '.') }}</span>
                        </div>
                        
                        <form action="{{ route('grow.builder.add-to-cart') }}" method="POST" class="mb-2">
                            @csrf
                            <button type="submit" class="btn btn-add-to-cart w-100" {{ $isEmpty ? 'disabled' : '' }}>
                                <i class="bi bi-cart-plus me-2"></i> Adicionar ao Carrinho
                            </button>
                        </form>
                        
                        <a href="{{ route('grow.builder.clear') }}" class="btn btn-clear w-100">
        ...
        <div class="col-lg-8 order-lg-1">
            <div class="accordion mb-4" id="accordionComponents">
                @forelse($categories as $index => $category)
                    <div class="mb-3">
                        <div class="category-header @if($index === 0 || (isset($selectedComponents) && !isset($selectedComponents[$category->id]))) active @endif" id="heading{{ $category->id }}">
                            <button class="category-toggle-btn" type="button" data-bs-toggle="collapse" data-bs-target="#collapse{{ $category->id }}" aria-expanded="{{ $index === 0 ? 'true' : 'false' }}" aria-controls="collapse{{ $category->id }}">
                                <div class="d-flex justify-content-between align-items-center">
                                    <span><i class="bi bi-chevron-right me-2"></i> {{ $category->name }}</span>
                                    @if(isset($selectedComponents[$category->id]))
                                        <span class="category-status selected">Selecionado</span>
                                    @else
                                        <span class="category-status pending">Pendente</span>
                        ...
                        <div id="collapse{{ $category->id }}" class="collapse @if($index === 0 || (isset($selectedComponents) && !isset($selectedComponents[$category->id]))) show @endif" aria-labelledby="heading{{ $category->id }}" data-bs-parent="#accordionComponents">
                            <div class="p-3">
                                <div class="row g-3">
                                    @forelse($category->components as $component)
                                        <div class="col-md-6 col-xl-4">
                                            <div class="component-card @if(isset($selectedComponents[$category->id]) && $selectedComponents[$category->id]['id'] == $component->id) selected @endif">
                                                <div class="component-card-header">
                                                    <h6 class="mb-0 text-truncate">{{ $component->name }}</h6>
                                                </div>
                                                <div class="component-img-container">
                                                    @if($component->image)
                                                        <img src="{{ asset('storage/' . $component->image) }}" alt="{{ $component->name }}" class="component-img">
                                                    @else
                                                ...
                                                <div class="p-3">
                                                    <div class="component-description mb-3">{{ $component->description }}</div>
                                                    
                                                    <div class="d-flex justify-content-between align-items-center">
                                                        <div class="component-price">R$ {{ number_format($component->price, 2, ',', '.') }}</div>
                                                        <form action="{{ route('grow.builder.select') }}" method="POST">
                                                            @csrf
                                                            <input type="hidden" name="category_id" value="{{ $category->id }}">
                                                            <input type="hidden" name="component_id" value="{{ $component->id }}">
                                                            <button type="submit" class="btn btn-select @if(isset($selectedComponents[$category->id]) && $selectedComponents[$category->id]['id'] == $component->id) selected @endif">
                                                                @if(isset($selectedComponents[$category->id]) && $selectedComponents[$category->id]['id'] == $component->id)
