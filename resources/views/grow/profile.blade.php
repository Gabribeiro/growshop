...
@section('title', 'Minha Conta - Potiguara Grow Shop')
            ...
            <div class="tab-content">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-4">
                                <div class="info-card">
                                    ...
                                    <div class="info-value">{{ auth()->user()->firstName }} {{ auth()->user()->lastName }}</div>
                                </div>
                            </div>
                    ...
                    <div class="profile-section">
                            ...
                            @foreach($orders as $order)
                                <div class="order-card">
                                    ...
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <div>
                                            <span class="order-status status-{{ $order->status == 'paid' ? 'paid' : ($order->status == 'canceled' ? 'canceled' : 'pending') }}">
                        ...
                        @else
                            <div class="no-orders">
                                ...
                                <a href="{{ route('grow.products') }}" class="btn btn-custom-primary mt-3">Explorar Produtos</a>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="tab-pane fade" id="addresses-tab-pane" role="tabpanel" aria-labelledby="addresses-tab" tabindex="0">
                    <div class="profile-section">
                        ...
                        @if(auth()->user()->addresses && count(auth()->user()->addresses) > 0)
                            <div class="row">
                                @foreach(auth()->user()->addresses as $address)
                                    <div class="col-md-6 mb-4">
                                        <div class="address-card {{ $address->default ? 'address-card-default' : '' }}">
                                            @if($address->default)
                                                <div class="default-badge">Padrão</div>
                                            @endif
                                            
                                            <h5>{{ $address->company }}</h5>
                                            ...
                                            @if($address->postal)
                                                <p class="mb-1">CEP: {{ $address->postal }}</p>
                                            @endif
                                            ...
                                            <div class="address-actions">
                                                ...
                                                @if(!$address->default)
                                                    <form action="/conta/enderecos/{{ auth()->user()->id }}/{{ $address->id }}" method="POST" class="d-inline">
                                                        @csrf
                                                        @method('DELETE')
